...
def tool_instruction_message(registry: ToolRegistry, native_tools: bool = False) -> dict[str, str]:
    ...
    desktop_lines = [
        ...
        "- If doctor reports blockers (accessibility, ydotool, portal, window listing), call enable_desktop_control to attempt enabling them, then re-run doctor to confirm.",
        "- Only claim a capability works if enable_desktop_control reports its status as 'enabled'; for 'needs_user'/'blocked' capabilities, relay the remediation to the user instead of asserting control.",
